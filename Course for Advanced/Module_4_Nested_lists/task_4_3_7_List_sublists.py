...
подсписок списка [1, 2, 3, 4], так как элементы 2 и 4 во втором списке не смежные (они разрываются элементом 3).
...
for j in range(len(symbols), 0, -1):  # кол-во влож. списков с одинаковым числом элементов
    for k in range(j):
        res.append(symbols[k:k + abs(j - (len(symbols) + 1))])
