...
в котором он продается. Вам доступен файл prices.csv, который содержит информацию о ценах продуктов в
различных магазинах. В первом столбце записано название магазина, а в последующих — цена на соответствующий
...
...
Напишите программу, которая определяет и выводит самый дешевый продукт и название магазина, в котором он
продается, в следующем формате:
...
Ответ на задачу доступен по ссылке. https://stepik.org/media/attachments/lesson/518491/clue_hungry_student.txt
...
with open('prices.csv', 'r', encoding='utf-8') as file:
    ...
    name_products = data[0][1:]
    result = []
    for row in data[1:]:
        prices = [int(x) for x in row[1:]]
        min_price = min(prices)
        name_chip_product = name_products[prices.index(min_price)]
        shop = row[0]
        result.append([shop, name_chip_product, min_price])

find_product = sorted(result, key=lambda x: (x[2], x[1], x[0]))[0]
print(f'{find_product[1]}: {find_product[0]}')