...
def is_it_open(work_time, visit_time):
    work_start, work_finish = [time.fromisoformat(x) for x in work_time.split('-')]
    visit_start, visit_finish = [time.fromisoformat(x) for x in visit_time.split('-')]
    return visit_start >= work_start and visit_finish <= work_finish
...
with open('pools.json', 'r', encoding='utf8') as file:
    data = json.load(file)
    result = filter(
        lambda x: is_it_open(x['WorkingHoursSummer']['Понедельник'], visit_time),
        data
    )
    result = list(sorted(
        result,
