...
Примечание 1. Функция должна изменять переданный словарь, а не возвращать новый. Возвращаемым значением
функции должно быть None.

Примечание 2. Гарантируется, что переданный в функцию словарь можно отсортировать, то есть он не содержит
ключи, имеющие разные типы, а также значения, имеющие разные типы.
...
Архив с тестами https://stepik.org/media/attachments/lesson/634520/tests_3084106.zip
GitHub https://github.com/python-generation/Professional/tree/main/Module_6/Module_6.6/Module_6.6.19
Sample Input 1:
...
def custom_sort(ordered_dict, by_values=False):
    ...
    if by_values:
        key_sort = 1
    data = sorted(ordered_dict.items(), key=lambda x: x[key_sort])
    for i in range(len(ordered_dict)):
        ordered_dict.move_to_end(data[i][0])
