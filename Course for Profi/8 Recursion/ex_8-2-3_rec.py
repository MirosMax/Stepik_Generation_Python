...
рекурсии, чтобы он вывел все элементы этого списка от первого до последнего, каждый на отдельный строке,
в следующем формате:
...
def num_list(numbers, step=0):
    if len(numbers) > 0:
        print(f'Элемент {step}: {numbers.pop(0)}')
        num_list(numbers, step + 1)
